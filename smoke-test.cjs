...
(async () => {
 ...
 async function read(){await page.evaluate(()=>{let guard=0;while(window.__check.dialogue&&!window.__check.dialogue.choices&&guard++<20)window.__check.advance();});}
 await visit('keeper',348,177);await read();assert(await page.evaluate(()=>window.__check.state.met));
 ...
 await page.evaluate(()=>{for(let i=0;i<3;i++)window.__check.advance();});
 ...
 await visit('keeper',348,177);await page.evaluate(()=>{for(let i=0;i<3;i++)window.__check.advance();});
