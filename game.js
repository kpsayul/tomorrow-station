...
const fresh=()=>({x:240,y:224,room:0,met:false,ticket:false,cat:false,bell:false,choice:null,ended:false,chapter:1,night2:freshNight2(),night3:freshNight3(),day4:freshDay4()});
...
let runToggle=false,saveMenuMode='save';
const entities=[
...
function roomEntities(){return state.chapter===4?fourthDay.roomEntities(state.room):state.chapter===3?thirdNight.roomEntities(state.room):secondNight.roomEntities(state.room);}
function move(room,x=240,y=230){state.room=room;state.x=x;state.y=y;keys.clear();objective();save();tone(293,.7);}
function stationTime(){return state.chapter===4?(state.day4.ended?'09:12':state.day4.lit?'08:40':'08:07'):state.chapter===3?(state.night3.ended?'06:12':state.night3.announced?'05:59':'00:10'):state.night2.ended?'00:09':state.ended?'00:08':'00:07';}
function nextChapter(){return state.chapter===1&&state.ended?2:state.chapter===2&&state.night2.ended?3:state.chapter===3&&state.night3.ended?4:null;}
function nextChapterLabel(){return nextChapter()===4?'바닷가 이야기 시작 →':nextChapter()===3?'세 번째 밤 시작 →':'두 번째 밤 시작 →';}
function floorY(room){return fourthDay.minY[room]??(room===1?173:138);}
function save(){try{localStorage.setItem(SAVE,JSON.stringify({...state,savedAt:new Date().toISOString()}));return true;}catch{return false;}}
function normaliseSave(s){if(s&&typeof s==='object'&&[0,1,2,3,4,5,6,7,8].includes(s.room)&&Number.isFinite(s.x)&&Number.isFinite(s.y)){
 ...
 const chapter=[2,3,4].includes(s.chapter)?s.chapter:1,room=chapter===4?(s.room>=6?s.room:6):Math.min(s.room,chapter===1?1:chapter===2?3:5);
 return {...fresh(),...s,chapter,room,night2:n,night3:p,day4:d,x:Math.max(29,Math.min(449,s.x)),y:Math.max(floorY(room),Math.min(270,s.y))};
}return null;}
...
function modalOpen(){return $('journal').open||$('save-menu').open;}
...
function saveSummary(snapshot){
 const places=['대합실','승강장','기록실','비 오는 기억','신호실','첫차 안','물결마을 항구','작은 식탁','등대 작업실'];let progress;
 if(snapshot.chapter===4){const n=snapshot.day4;progress=n.ended?'바닷가 이야기 완료':n.reunited?'03호에게 엽서 보내기':n.lit?'여울과 나루의 이야기':n.met?'등대 빛길 퍼즐':n.letter?'나루를 만나러':n.notice?'반송된 편지':'마을에 도착';}
 ...
 return `${['첫 번째 밤','두 번째 밤','세 번째 밤','바닷가 이야기'][snapshot.chapter-1]} · ${places[snapshot.room]} · ${progress}`;
}
...
function restore(snapshot,label){
 ...
 if($('journal').open)$('journal').close();if($('save-menu').open)$('save-menu').close();
 closeDialogue();objective();save();$('save-status').textContent=`${label} 불러옴`;toast(`${label}에서 이어갑니다.`);canvas.focus({preventScroll:true});
}
...
function openSaveMenu(mode){
 ...
 renderSaveMenu();$('save-menu').showModal();
}
...
function objective(){
 const task=currentTask(),station=state.chapter>=3?thirdNight.stationName():'내일역';$('location').textContent=`${state.room===3?'오래전':stationTime()} · ${[`${station} 대합실`,'2번 승강장','잊힌 소리 기록실','비가 내리던 역','신호실','첫차 안','물결마을 항구','작은 식탁','등대 작업실'][state.room]}`;
 $('objective').textContent=task.title;$('guide').hidden=!active;$('guide-step').textContent=task.step;$('guide-title').textContent=task.title;
 $('guide-detail').textContent=task.detail+((state.chapter===4?state.day4.ended:state.chapter===3?state.night3.ended:state.chapter===2?state.night2.ended:state.ended)?'':' 가까이서 E / Enter · 대화는 끝까지 읽어주세요.');
 $('next-night').hidden=!(active&&nextChapter());$('next-night').textContent=nextChapterLabel();
 $('chapter-label').textContent=['01 / 기억이 머무는 역','02 / 받는 이 없는 편지','03 / 첫차가 오는 방법','04 / 약속의 다른 쪽'][state.chapter-1];
...
function interact(){if(!active||!$('ending').hidden||modalOpen())return;if(conversation){advance();return;}const e=closest();if(!e){toast('조금 더 가까이 다가가 보세요.');return;}
 if(fourthDay.handle(e.id)||thirdNight.handle(e.id)||secondNight.handle(e.id))return;
 switch(e.id){
 case 'keeper':
...
$('next-night').onclick=$('end-next-night').onclick=()=>{const next=nextChapter();if(!next)return;$('ending').hidden=true;closeDialogue();if(next===4)fourthDay.start();else if(next===3)thirdNight.start();else secondNight.start();};
$('journal-button').onclick=()=>{keys.clear();$('journal-content').replaceChildren();let notes;
 ...
 for(const n of notes){const p=document.createElement('p');p.textContent=n;$('journal-content').append(p);}$('journal').showModal();};$('close-journal').onclick=()=>{$('journal').close();canvas.focus({preventScroll:true});};
...
function drawLobby(){rect(0,0,W,H,'#1a2a3a');rect(20,47,440,113,'#30434e');for(let x=20;x<460;x+=32){rect(x,47,1,107,'#394b54');}rect(20,47,440,4,'#718077');rect(20,154,440,5,'#7e8170');rect(20,159,440,118,'#3d4b50');for(let y=165;y<280;y+=22){rect(20,y,440,1,'#53605d');for(let x=20+(y%2)*12;x<460;x+=32)rect(x,y,1,22,'#46565a');}rect(0,277,480,11,'#142330');rect(16,48,7,230,'#1b2c3a');rect(458,48,7,230,'#1b2c3a');
 for(const x of [128,266]){rect(x,68,76,58,'#182c40');rect(x+3,71,70,51,'#23394e');rect(x+36,70,3,54,'#6b7c7c');rect(x+3,95,70,3,'#617575');rect(x+5,117,66,3,'#435e65');for(let j=0;j<9;j++){const px=x+6+(j*19)%65,py=74+(j*13)%39;rect(px,py,1,1,'#a5c4c9');}rect(x-3,126,82,4,'#8a9280');}
 ...
 rect(316,143,72,26,'#22313a');rect(312,140,80,7,'#a18d6a');rect(317,149,70,17,'#52615d');rect(322,151,59,12,'#465551');text('분실물 보관소',351,159,'#f0eed4',6,'center');if(!(state.chapter===3&&state.night3.ended))person(348,141,true);rect(327,135,14,4,'#d4cbb3');rect(328,132,10,3,'#a7b6ad');
 ...
 secondNight.drawLobbyDoor();
}
function drawPlatform(){rect(0,0,W,H,'#182a3e');for(let i=0;i<64;i++){let x=(i*71+13)%W,y=(i*37)%113;rect(x,y,1,1,i%4?'#526e86':'#b9c5ba');}glow(389,48,58,'#a4c5c61a');rect(382,37,15,15,'#c9d7c4');rect(389,35,12,13,'#182a3e');for(let i=0;i<15;i++){let x=i*37;rect(x,97-(i%4)*7,27,46,'#203344');rect(x+8,107-(i%4)*7,3,4,'#7b86684d');}
...
function allowed(x,y){const blocks=fourthDay.obstacles[state.room]||thirdNight.obstacles[state.room]||secondNight.obstacles[state.room]||obstacles[state.room];return x>=29&&x<=449&&y>=floorY(state.room)&&y<=270&&!blocks.some(o=>x>o.x-5&&x<o.x+o.w+5&&y>o.y-1&&y<o.y+o.h+4);}
