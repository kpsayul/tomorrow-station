...
window.createSecondJourney=function(api){
 ...
 function captain(){
  ...
  speak('기관사 · 이음',[
   '이음: 동생 결이 보낸 마지막 말은 ‘돌아오지 마’였어요. 일곱 해째 그걸 들어요.\n원본은 중앙국에 있어요. 당신 역의 반송 기록도 거기로 가고요.',
   `이음: 당신처럼 누군가 기다리며 만든 역들이에요. 여긴 아직 내일역이네.\n이야기: 지도에 ${state().night3.choice==='rest'?'쉼표역':'다음역'}을 적으려던 펜이 멎었다. 주소 정정 요청까지 반송돼 있었다.`,
   '이음: 나는 빈 열차를 몰아요. 승객이 타면 무단 운행이거든.\n후추: 지금 우리 탔는데.\n이음: 그래서 먼저 이름을 알려드린 거예요.',
   '이음: 지도에 없다는 이유로 못 서는 역이에요. 사람이 있는데도.\n이야기: 차창 밖 아이가 지도를 흔들었다. 이음은 경적을 두 번 울렸지만 지나쳤다.',
   '이음: 철교를 열어야 해요. 소포, 장부, 03호 수신표를 비교해주세요.\n실제 요청과 반송 명령이 다르다는 증거가 필요해요.'
  ],()=>commit(()=>{n.met=true;}));
 }
 function clue(id){
 ...
 function clerk(){const n=p().seven;if(!n.met){speak('중앙국장 · 서린',['“먼저 저 아이 얘기부터 듣고 오세요.\n내 설명을 먼저 들으면 지도 대신 규정을 보게 될 테니까.”']);return;}
  ...
  speak('중앙국장 · 서린',[
   ...
   '03호: 편지를 막아놓고 답을 기다렸군요.\n이야기: 서린은 반박하는 대신 서류함을 열었다.',
   '서린: 결재철과 운행 기록을 대조하세요. 제 서명도 그대로 있어요.'
 ...
 function appeal(){const n=p().seven;
  ...
  speak('이의 신청 단말',['서린이 묻는다. “폐선이 부당하다는 근거가 무엇입니까?”\n서명란 옆에는 자동 반송 버튼이 켜져 있다.'],null,[
   ...
   {text:'보류 신청이 거절로 바뀌었다.',action:()=>speak('중앙국장 · 서린',[
    ...
    '서린: 안 읽었어요. 아무도 답하지 않는 노선이라고 믿었으니까.\n이음: 결도? 내 동생 것도?',
 ...
 function director(){const n=p().eight;if(n.met){speak('중앙 우편국',[n.live?'이음: 기다릴 수 있어. 이제 어디를 보고 기다리는지 아니까.\n이야기: 이음은 수신기의 불을 보고 있었다.':'서린: 가운데 음성 원본부터 들읍시다.\n이야기: 서린이 자기 의자를 이음 쪽으로 밀었다.']);return;}
  speak('중앙 우편국',[
   '서린: 사고 당일엔 첫 문장만 전달됐어요. 복구된 원본은 나중에 왔죠.\n나는 폐선한 노선의 추가 기록을 열어보지 않았습니다.',
   '이음: 그동안 나는 동생이 날 안 만나겠다는 말을 매일 들었어요.\n이야기: 서린은 변명하려다 멈췄다.',
   '이음: 사과는 나중에 들어요. 지금은 이 목소리부터.\n이야기: 이음이 직접 재생 버튼에 손을 댔다.'
  ],()=>commit(()=>{n.met=true;}));
 }
 ...
 function finale(){const n=p().eight;if(n.ended){speak('새벽의 출발 신호',['열일곱 노선이 저마다 다른 시각에 깨어난다.\n이번 여정은 마쳤다. 친구들과 남은 편지를 살펴보자.']);return;}
  ...
  speak('새벽의 연결교',[
   ...
   '이야기: 우리 역에서 여울의 전화가 왔다.\n여울: 기다리는 손님 있어요?\n당신: 여기는 지금, 내려서 만나는 중이에요.'
 ...
 function epilogue(id){
  ...
  else if(id==='home-letter')speak('우리 역에서 온 편지',[state().day4.choice==='table'?'나루: 언니가 이번 목요일에는 밥을 하겠대. 모래가 걱정하더라.':'나루: 이번 목요일에는 내가 역으로 갈게. 길은 언니한테 물어봤어.','나루: 파란 컵 손잡이는 내가 다시 고칠게.\n언니가 자꾸 양손으로 들어서. 이번에는 제대로 붙이려고.',state().night5.guestChoice==='quiet'?'연: 조용히 앉아 있다가 편지 분류를 조금 도왔어요. 다음엔 그냥 놀러 올게요.':'연: 다음 손님이 와서 내 얘기보다 그분 얘기를 더 들었어요. 물은 03호가 줬고요.','백지: 휴가 이틀 연장했습니다. 새 노선으로 돌아갑니다.\n03호: 승인자 칸도 채웠습니다. 이번에는 물어보고요.']);
