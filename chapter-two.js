...
window.createSecondNight = function createSecondNight(api) {
 ...
 function start(){
  ...
  speak('역내 방송',[
   "서랍 안에서 종이 찢어지는 소리가 났다.\n봉투의 글씨가 지워지고 있었다. 마지막 문장은 「읽지 않은 것으로 해주세요.」",
   "여울: 어제 당신이 맡긴 편지예요. 지금도 돌려받기 싫은가 봐요.\n이야기: 그녀가 봉투를 잡고 있는 동안에도 글자 하나가 더 사라졌다."
 ]);
 }
 ...
 function puzzle(){
  ...
  const options=['rain','whistle','train'].filter(id=>!n.sequence.includes(id)).map(id=>({text:names[id],action:()=>{
   ...
   if(n.sequence.join(',')==='rain,whistle,train'){
    speak('기억 재생기',['빗소리 → 휘파람 → 열차.\n맞물린 순간, 재생기 밖으로 빗물이 흘러나왔다.'],()=>{n.tuned=true;move(3,240,237);toast('두 번째 밤 · 비가 그치기 전');});
 ...
 function finalLetter(){
  ...
  speak('역무원 · 여울',[
   ...
   "어린 온의 편지: 아빠는 결국 왔어. 빵도 있었어.\n그런데 내가 왜 울었는지는 몰랐어.",
   "어린 온의 편지: 아빠가 집에 가자고 했는데 발을 못 뺐어.\n후추가 내 신발 위에서 자고 있었거든.",
   "어린 온의 편지: 그래서 아빠도 같이 앉았어. 빵이 다 식을 때까지.\n바다 가면 꼭 적어줘. 후추 말고 네 얘기도.",
   "이야기: 봉투 뒷면에는 어제의 당신이 남긴 답이 있었다.\n어제의 온: 미안. 오늘은 못 가. 내일 갈게.",
   "그 아래에도, 그 아래에도 같은 문장.\n연필은 마지막 줄에서 종이를 뚫었다.",
 ...
 function end(choice){
  ...
  speak('역무원 · 여울',choice==='name'?[
   ...
   "당신: 이름 없이도 돌려받은 걸로 해주세요.\n이야기: 후추가 빈 이름 칸 옆에 발자국을 눌렀다.",
   "안내판에서 소리가 났다.\n「마지막 보관품: 주인 확인 완료. 영업 종료 절차 시작.」"
 ...
 function handle(id){
  ...
  switch(id){
   ...
   "여울: 이름을 맡기면 그 이름으로 살던 일도 흐려져요. 당신이 원했어요.\n저도 붙잡았고요. 혼자 남기 싫어서.",
   "여울: 제 말만 믿지 마세요. 기록실에 당신 표와 번호가 같은 물건 셋이 있어요.\n이야기: 여울이 열쇠를 당신 쪽으로 밀었다."
   ...
   case 'rain':speak('노란 우산',[
    '우산을 펴자 비가 들린다. 처음엔 비밖에 없었다.\n아직 열차도, 호루라기 소리도 없다.',
    '우산을 쓴 아이: 혼자 기다리지 말랬는데, 기다리는 걸 누구랑 하지?\n이야기: 벤치 아래에서 작은 발이 보였다.'
   ],()=>recordClue('rain'));return true;
   case 'whistle':speak('작은 호루라기',[
    '아이가 호루라기를 불자 비를 피하던 고양이가 다가왔다.\n아직 열차는 오지 않았다.',
    '우산을 쓴 아이: 야, 나도 혼자야. 같이 있으면 혼자는 아닌 거지?'
   ],()=>recordClue('whistle'));return true;
   case 'train':speak('장난감 열차',[
    '열차가 들어왔다. 아이와 고양이는 이미 나란히 앉아 있었다.\n장난감의 고친 바퀴만 달그락거렸다.',
    '우산을 쓴 아이: 우리 데리러 온 거 아니야. 조금 더 같이 있어도 돼.'
   ...
   "우산을 쓴 아이: 아빠가 보냈어? …아니구나. 아빠는 빵을 사 오거든.\n이야기: 아이는 당신의 빈손을 보고 우산을 조금 내렸다.",
   "“자리 있어. 얘는 후추야. 나는 온.”\n고양이가 당신 자리까지 차지하고 누웠다.",
   "우산을 쓴 아이: 너는 누구 기다려?\n이야기: 아이 가방 속 봉투의 글씨가 당신의 것과 같았다."
 ],null,[
   ...
   "후추: 이름은 네 부탁 때문에 숨겼어. 방울은 여울이 숨겼고.\n난 둘 다 알면서 말을 안 했어.",
   "당신: 알면서 기다리게 한 거야?\n이야기: 후추는 한참 바닥을 봤다.\n후추: …응.",
   "후추: 나는 네가 맡긴 기억에 남은 고양이야. 어른이 된 네 일은 잘 몰라.\n그래도 이번엔 못 들은 척 안 할게. 편지는 네가 정해."
 ...
 function childReply(answer){
  speak('우산을 쓴 아이',[
   ...
   "우산을 쓴 아이: 나중의 나한테 편지 쓰는 숙제야. 부탁을 써도 될까?\n이야기: 아이와 포갠 손의 흉터가 같은 모양이었다.",
   "우산을 쓴 아이: 이렇게 두 번. 여기 있다고 알려주는 거야.\n이야기: 당신도 벤치를 두드렸다. 고양이가 번쩍 눈을 떴다.",
   "우산을 쓴 아이: 너무 세. 얘한텐 천둥이겠다.\n이야기: 이번에는 손끝으로 아주 작게. 아이가 웃었다. 고양이는 깨지 않았다.",
