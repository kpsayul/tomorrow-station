...
window.createFourthDay = function createFourthDay(api) {
 ...
 function owner(){
  ...
  speak('식당 주인 · 모래',[
   ...
   "여울: 안 읽었어요. 화난 말이 있을 것 같아서.\n내일 받겠다고 했어요. 내일은 읽겠다고.",
   "모래가 마지막 봉투를 돌려줬다.\n“처음엔 배 오는 날마다 국 두 그릇을 시켰어요.”",
   "“나중엔 한 그릇만 시켰고요.\n열세 번째 편지는 안 썼어요.”",
   "이야기: 여울은 봉투를 열려다 손을 멈췄다.\n여울: 지금은 어디 있어요?\n모래: 등대요. 올라가요. 기다려줄지는 직접 물어보고."
 ...
 function naru(){
  ...
  if(!p.lit){speak('나루',[
   ...
   '여울: 이 장갑이면 돼?\n이야기: 나루가 고개만 끄덕였다. 처음으로 여울에게 손을 내밀었다.'
  ]);return;}
  speak('여울과 나루',[
   "여울: 세 번째 편지에 병원 주소가 있었어.\n나루: 사다리에서 떨어졌을 때. 언니 번호도 적었었어.",
   "나루: 옆 침대 사람 언니가 귤을 가져왔더라. 나도 하나 받았어.\n그때는 나도 그냥 언니가 왔으면 좋겠더라.",
   "여울: 네가 화났을까 봐 안 읽었어. 미안해. 다음 편지는…\n나루: 안 쓸 거야. 열세 번째는 언니가 써.",
   "여울: 목요일에 내가 전화해도 돼?\n나루: 여섯 시 이후. 일할 때는 안 받아.\n이야기: 여울은 봉투에 시간을 적었다.",
   "나루: 밥 먹는다고 다 괜찮아진 건 아니야. 그래도…\n같이 먹을래? 아니면 언니랑 잠깐만 걸을까."
 ],null,[
 ...
 function reunite(choice){
  const cups=[
   '여울: 이거, 첫 번째 편지에…\n이야기: 나루가 선반에서 컵 두 개를 내렸다. 파란 컵에는 서툰 고래와 오래된 접착 자국이 있었다.',
   '나루: 응. 나중엔 여기로 가져왔어. 손잡이는 잡지 마. 다시 떨어져.\n이야기: 여울이 컵을 두 손으로 감쌌다.',
   '나루: 버리려고 했는데. 자꾸 다음에 버리게 되더라.\n이야기: 컵에는 아직 아무것도 담기지 않았다. 여울은 그대로 들고 있었다.'
  ];
  speak('길을 나서는 사람들',[...cups,...(choice==='table'?[
   "후추: 고양이도 자리 있어?\n나루: 밥값 낼 거야?\n후추: 분위기가 안 좋을 땐 그런 얘기 안 하는 거야.",
   "여울이 나루 옆 의자를 당기려다 멈췄다.\n나루가 맞은편 의자를 발로 밀어줬다.",
   "나루: 거기 앉아. 얼굴은 봐야 할 거 아냐.\n이야기: 모래가 두 사람이 가져온 컵에 따뜻한 우유를 따랐다.",
   "여울은 제 컵을 몇 번 불고는 나루 쪽으로 밀었다.\n나루: 나 이제 뜨거운 거 먹을 줄 알아.",
   "여울의 손이 멈췄다.\n나루는 컵을 돌려주려다 거품만 한 입 먹었다.",
   "나루: 그래도 언니 거가 더 맛있네.\n이야기: 여울이 웃으려다 고개를 숙였다. 모래는 잠깐 주방에 들어갔다."
 ]:[
   "나루: 등대 아래까지만.\n여울: 응.\n나루: 언니, 자꾸 짐 가져가려고 하지 마. 내가 들게.",
   "두 사람은 나란히 걷지 못했다.\n모퉁이에서 나루가 잠깐 멈춰 기다렸다.",
   "모래가 문밖까지 나와 두 컵에 보리차를 따라줬다.\n여울은 파란 컵을 두 손으로 감싸고 다시 걸었다.",
   "여울: 고래 잘 그렸네.\n나루: 감자인데.\n이야기: 여울은 한 걸음 늦게 웃었다.",
   "이야기: 여울이 빈 컵을 돌려주려 하자 나루가 고개를 저었다.\n나루: 언니 거야. 다음에 올 때 가져와.",
   "후추: 우린 밥 먹자. 따라가서 해설까지 해주지는 말고."
 ...
 function postbox(){
  ...
  speak('03호에게 보내는 엽서',[
   ...
   p.choice==='table'?'당신의 엽서: 오늘은 두 사람이 서로 컵을 바꿔 썼어.\n후추는 자기 우유가 없다고 항의했고. 사진 찍을 틈을 놓쳤네.':'당신의 엽서: 두 사람은 등대 아래까지 걸었어.\n나루가 여울에게 컵을 줬어. 다음에 가져오라고.',
 ...
 function handle(id){
  ...
  switch(id){
   ...
   case 'cafe-menu':speak('오늘의 메뉴',['생선국 · 달걀말이 · 따뜻한 밥\n반찬 추가 무료, 사연 추가도 무료.','이야기: 메뉴 밑에 낙서와 답장이 있다.\n후추: 고양이 할인은 왜 없어요.\n모래: 글씨 쓸 줄 알면 계산도 하렴.']);return true;
