...
(async()=>{
 ...
 try{
  ...
  // The continuation emits distinct chapter milestones for 6, 7 and 8.
  const continuation=await setup({configured:true});await continuation.page.locator('#begin').click();
  for(const chapter of [5,6,7]){
   await continuation.page.evaluate(chapter=>{const g=window.__growth,s=g.state;s.chapter=chapter;s.room=chapter===5?1:chapter*2-1;if(chapter===5)s.night5.ended=true;else s.journey2[chapter===6?'six':'seven'].ended=true;g.showEnding({eyebrow:'test',title:'test',body:'test',next:true});},chapter);
   await continuation.page.locator('#end-next-night').click();await continuation.page.keyboard.press('Escape');
  }
  const newMilestones=await continuation.page.evaluate(()=>window.dataLayer.filter(e=>e[0]==='event'&&e[1]==='chapter_start').map(e=>e[2].chapter));assert.deepEqual(newMilestones,[1,6,7,8]);await continuation.context.close();
